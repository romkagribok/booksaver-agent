...
class CheckCoordinator:
    ...
    def _default_browser_factory() -> AbstractContextManager[Any]:
        ...
        return PlaywrightInteractiveBrowser(headless=True)
    ...
    def _run_booking(
        ...
        monitor = BookingComSearchMonitor(
            ...
            session_manager=SessionManager(
                LocalSessionRepository(self._config.data_directory)
            ),
        ...
        results = monitor.run_all_active(bookings=[booking])
        ...
        if results:
            ...
            result = CheckResult.failure(
                ...
                datetime.now(UTC),
                FailureReason(
                    code=FailureCode.NAVIGATION_ERROR,
                    detail="Monitor could not initialize a live browser check.",
                ),
            )
            history.add(result)
